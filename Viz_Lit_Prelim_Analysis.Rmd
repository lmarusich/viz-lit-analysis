...
pacman::p_load(tidyverse, knitr, ez, psychReport, superb, plotrix, emmeans
...
## Compare conditions for question type (instead of by individual question)
...
read_rt_type_wider <- trial_type_means %>%
  ...
  pivot_wider(names_from = TrialType,values_from = mean_readRT, names_prefix="Type")
answer_rt_type_wider <- trial_type_means %>%
  select(ParticipantId, Condition, TrialType, mean_answerRT) %>%
  pivot_wider(names_from = TrialType,values_from = mean_answerRT, names_prefix="Type")
...
read_rt_type_anova <- ezANOVA(trial_type_means,
                              dv = mean_readRT,
                              wid = ParticipantId,
                              within = TrialType,
                              between = Condition,
...
#get the partial eta-squared too
aovEffectSize(read_rt_type_anova, effectSize = "pes")$ANOVA
...
answer_rt_type_anova <- ezANOVA(trial_type_means,
                              dv = mean_answerRT,
                              wid = ParticipantId,
                              within = TrialType,
                              between = Condition,
...
#get the partial eta-squared too
aovEffectSize(answer_rt_type_anova, effectSize = "pes")$ANOVA
